...
import { Gamepad, Puzzle, Gift} from "lucide-react";
      ...
      content: (
        <div className="relative h-full w-full overflow-hidden bg-hero-gradient">
          ...
          <div className="relative h-full flex flex-col items-center justify-center text-center px-6">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-primary/50 px-4 py-2 text-primary font-cyber backdrop-blur-sm">
              <Star className="h-4 w-4" />
              Change Your Perspective
            </div>
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-cyber font-black leading-[0.95]">
            ...
            <Button 
              size="lg" 
              className="mt-6 bg-gradient-to-r from-primary to-accent text-background font-cyber font-bold px-8 py-6 shadow-accent-glow hover:opacity-90 transition-opacity cursor-pointer"
              onClick={() => {
            ...
            >
              <Rocket className="h-5 w-5 mr-2" /> Join the Experience
            </Button>
          </div>
        </div>
      ),
    },
    {
        ...
        content: (
            <div className="relative h-full w-full overflow-hidden bg-cosmic-gradient bg-flowing-ribbons">
                ...
                <div className="relative h-full flex flex-col items-center justify-center text-center px-6 py-4 overflow-auto">
    
    
                    <h1 className="text-3xl md:text-4xl lg:text-5xl font-cyber font-black leading-tight tracking-tight">
                        <span className="block text-transparent bg-clip-text bg-neon-gradient">🎮 GAMES • 🧩 FUN • 🎁 PRIZES</span>
                        {/* <span className="block text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">FOR EVERYONE!</span> */}
                    </h1>
    
                    <p className="mt-6 max-w-4xl text-lg md:text-xl text-muted-foreground leading-relaxed">
                        ...
                        {/* Whether you're a developer, designer, or completely new to tech - this workshop is designed for <span className="text-accent font-bold">everyone</span>! */}
                    </p>
    
                    {/* Feature Highlights */}
                    <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full max-w-5xl">
                        {[
                            { icon: <Gamepad className="h-8 w-8 text-primary" />, title: "Fun Games", desc: "Interactive challenges that make learning addictive", animation: "animate-spin-glow" },
                            { icon: <Puzzle className="h-8 w-8 text-accent" />, title: "Creative Quests", desc: "Solve puzzles and unlock new levels of understanding", animation: "animate-heartbeat" },
                            { icon: <Gift className="h-8 w-8 text-primary" />, title: "Awesome Prizes", desc: "Win cool gifts for solving challenges and participating", animation: "animate-pulse-scale" },
                            { icon: <Users className="h-8 w-8 text-accent" />, title: "Team Adventures", desc: "Work together in exciting collaborative missions", animation: "animate-rainbow-glow" },
                            { icon: <Heart className="h-8 w-8 text-primary" />, title: "Empathy", desc: "Creating digital experiences that care", animation: "animate-pulse-scale" },
                            { icon: <Volume2 className="h-8 w-8 text-accent" />, title: "Accessibility Focus", desc: "Learn through hands-on accessibility adventures", animation: "animate-wave" },
                        ].map((item, idx) => (
                            <div key={idx} className="flex items-start gap-3 bg-background/60 backdrop-blur-sm rounded-xl p-4 border border-primary/20 hover:border-primary/40 hover:shadow-accent-glow transition-all duration-300 hover:scale-105">
                   ...
                   {[
                     ...
                     return (
                       <div key={idx} className="group relative">
                         <div className="text-center rounded-xl border-2 border-primary/20 bg-gradient-to-br from-primary/5 to-accent/5 p-3 hover:border-primary/40 hover:from-primary/10 hover:to-accent/10 transition-all duration-300 hover:shadow-accent-glow">
                           <div className="relative mx-auto mb-2 h-16 w-16">
                             ...
                             <div className="relative h-full w-full rounded-full border-2 border-primary/30 bg-speaker-card flex items-center justify-center shadow-secondary-glow group-hover:scale-[1.05] transition-transform duration-300 overflow-hidden">
                               ...
                               />
                               <User className="h-8 w-8 text-accent hidden" />
                             </div>
                           </div>
                           <div className="font-semibold text-primary tracking-wide text-sm">{speaker.name}</div>
                           <div className="text-xs text-accent mt-0.5">{speaker.jobTitle}</div>
                         </div>
                         
                         {/* Hover Card with conditional positioning */}
                         <div className={`absolute w-80 bg-gradient-to-br from-slate-900 to-slate-800 backdrop-blur-xl border-2 border-slate-600 rounded-2xl shadow-2xl opacity-0 scale-75 rotate-2 group-hover:opacity-100 group-hover:scale-100 group-hover:rotate-0 transition-all duration-500 ease-out z-50 pointer-events-none group-hover:pointer-events-auto ${getCardPosition(idx)}`}>
